...
def backtest(data_obj, strategy):
    """
    执行回测
    
    Args:
        data_obj: Data类的实例，包含股票数据
        strategy: 策略对象，必须包含init和onbar方法
    """
    # 获取所有股票数据
    all_data = data_obj.get()
    ...
    # 获取所有时间戳（基准时间序列）
    benchmark_timestamps = _get_benchmark_timestamps(all_data)
    ...
    for timestamp in sorted(benchmark_timestamps):
        # 生成当前时间戳的横截面数据
        cross_section_data = _create_cross_section_data(all_data, timestamp)
        ...
        if hasattr(strategy, 'onbar'):
            strategy.onbar(timestamp, cross_section_data)

def _get_benchmark_timestamps(all_data: Dict[str, pd.DataFrame]) -> List[datetime]:
    """
    获取基准时间戳序列
...
def _create_cross_section_data(all_data: Dict[str, pd.DataFrame], timestamp: datetime) -> pd.DataFrame:
    """
    创建横截面数据
