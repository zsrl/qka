...
```
           ...
           └─────────────────────────────────────┘
              │        │              │
              ▼        ▼              ▼
        self.get()  self.history()  self.ta.xxx()
        (横截面)    (时间序列)       (技术指标)
```
...
`self.ta` 提供内置技术指标，一行即可计算：

```python
# 简单指标 → pd.Series（当前横截面）
sma_20 = self.ta.sma('close', length=20)
ema_14 = self.ta.ema('close', length=14)
rsi_14 = self.ta.rsi('close', length=14)
atr_14 = self.ta.atr(high='high', low='low', close='close', length=14)
...
所有指标基于 `ta` 库（纯 Python，无额外二进制依赖），支持 43 个技术指标。详见 [first-strategy 进阶 2](first-strategy.md)。
